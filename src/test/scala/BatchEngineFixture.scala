...
class BatchEngineFixture extends munit.FunSuite {
  val engine = new Fixture[SQLiteBatchEngine] ("interactive engine"){
    var engine: SQLiteBatchEngine = null
    def apply() = engine
    override def beforeEach(context: BeforeEach): Unit = {
      ...
      engine = new SQLiteBatchEngine(jdbc_prefix, tcode_tbl_path, mazegaki_path, bushu_path) with QwertyLayout
    }
